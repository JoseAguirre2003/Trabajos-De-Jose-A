...
namespace ProyectoV2
{
	public partial class FormClient : Form
	{
		...
		public FormClient(ColeccionClientes col)
		{
			...
			coleccionClient = col;
			datePickerNacimiento.Value = DateTime.Now.AddYears(-18);
		}
		...
		public void btnBuscarClick(object sender, EventArgs e){
			if(txtCedula.Text.Trim() != ""){
				...
				if(pos > -1){
					...
					MessageBox.Show("No se pudo encontrar al cliente que busca", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}else
		...
		public void btnActualizarClick(object sender, EventArgs e){
			if(validar(false)){
				...
				String numeroCasa = txtTelefono.Text;
				DateTime fechaRegist = dateTimeRegistro.Value;
				if(coleccionClient.Actualizar(cedula, nombre, apellido, fechaNacim, direccion, correo, numeroMovil, numeroCasa, fechaRegist))
