...
namespace ProyectoV2.Classes
{
	public class ColeccionClientes
	{
		...
		public bool Actualizar(String cedula, String nombres, String apellido, DateTime fechaNacim, String direccion, String correo, String numeroMovil, String numeroCasa, DateTime fechaRegist){
		...
		public String escribir(Cliente client){
			...
			l += client.FechaNacim.ToString("s") + s + client.Direccion.ToString() + s + client.Correo.ToString() + s;
			l += client.NumeroCasa.ToString() + s + client.NumeroMovil.ToString() + s + client.FechaRegist.ToString("s") + "\n";
			return l;
		}
		...
		public bool recuperar(){
			if(File.Exists(ruta)){
				...
				foreach (String linea in allLines){
					...
					nombre = datos[1];
					apellido = datos[2];
					fechaNacim = Convert.ToDateTime(datos[3], CultureInfo.InvariantCulture);
					direccion = datos[4];
					correo = datos[5];
					numCasa = datos[6];
					numMovil = datos[7];
					fechaRegist = Convert.ToDateTime(datos[8], CultureInfo.InvariantCulture);
