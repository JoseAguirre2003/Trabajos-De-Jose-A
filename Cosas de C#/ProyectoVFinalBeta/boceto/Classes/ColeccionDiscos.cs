...
namespace ProyectoV2.Classes
{
	public class ColeccionDiscos
	{
		...
		public String escribir(DVD dvd){
			...
			l = dvd._code.ToString() + s + dvd._title.ToString() + s + dvd._fabricDate.ToString("s") + s;
			l += dvd._type.ToString() + s + dvd._descrip.ToString() + s + dvd._cantidad.ToString() + s;
			l += dvd._ingresDate.ToString("s") + s + dvd._imgRuta.ToString() + "\n";
			return l;
		}
		...
		public bool recuperar(){
			if(File.Exists(ruta)){
				...
				foreach (String linea in allLines){
					...
					code = datos[0];
					title = datos[1];
					fabricDate = Convert.ToDateTime(datos[2], CultureInfo.InvariantCulture);
					type = datos[3];
					descrip = datos[4];
					cantidad = Convert.ToInt16(datos[5]);
					ingresDate = Convert.ToDateTime(datos[6], CultureInfo.InvariantCulture);
					imgRuta = datos[7];
